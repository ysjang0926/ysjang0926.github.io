...
본 논문에서는 딥러닝 모델의 설명 및 예측을 위한 대표적인 2가지 해석 기법을 소개합니다.
* Sensitivity Analysis(SA)
* Layer-Wise Relevance Propagation(LRP)

AI 시스템의 예측을 설명하는 프로세스는 아래의 그림에 요약되어 있습니다.
* 먼저 AI 시스템은 입력 이미지를 '수탉'으로 올바르게 분류합니다.
* 그런 다음 입력 변수 관점에서 해당 결정에 도달한 이유를 설명하기 위해 SA 또는 LRP와 같은 방법을 적용합니다.
* 이 설명의 결과는 예측에 대한 각 픽셀의 중요성을 시각화한 히트맵입니다.
	* 이 예에서 수탉의 붉은 빗과 주름살은 AI 시스템의 결정을 위한 기초입니다.
	* 히트맵을 사용하면 AI 시스템이 의도한대로 작동하는지 확인할 수 있습니다.
...
딥러닝 모델에서 국소적(local)인 입력 변화에 대한 예측 결과의 변화량을 정량화하여 입력 이미지의 어떤 부분이 딥러닝 모델의 결과 도출에 큰 영향을 미쳤는지 설명하는 방법입니다. 즉, 모델의 locally evaluated gradient를 기반으로 한 예측을 설명합니다. <br>
SA는 각 입력 변수 $i$ (ex: 이미지 픽셀)의 중요도를 다음과 같이 편미분의 norm으로 계산합니다.
$R_i = || \frac{\partial}{\partial x_i}  f(x)||$
여기서 편미분은 network의 레이어를 통해 역전파 방법을 실행하고 연쇄법칙을 사용하여 얻을 수 있습니다.
$\frac{\partial f}{\partial x^{(l)}}=\frac{\partial x^{(l+1)}}{\partial x^{(l)}}\frac{\partial f}{\partial x^{(l+1)}}$

이때 가장 관련성이 높은 입력 특징(input feature)이 가장 민감한 특징이라 가정합니다.  SA는 함수값 $f(x)$ 자체를 설명하는 것이 아니라 함수 값의 변동을 설명합니다. (변동 = 이미지의 예측값을 높이거나 낮추는 것은 무엇인지)
SA로 계산된 히트맵은 이미지를 예측된 클래스와 비슷하거나 덜 비슷하게 보이게 하기 위해 어떤 픽셀을 변경해야 하는지를 나타냅니다.

예를 들어, 위의 그림에서 이 픽셀들은 수탉의 일부를 가리고 있는 노란꽃 부분입니다. 해당 픽셀을 특정 방법으로 바꾸면 수탉의 가려진 부분을 재구성할 수 있으며, 이를 통해 수탉이 더 많이 보일 것이기 때문에 분류 평가점수(classification score)를 높일 수 있습니다. 하지만 히트맵은 수탉 예측에 실제로 어떤 픽셀이 중요한지 나타내지 않는 단점이 있으며, 이 특성으로 AI 시스템의 예측을 설명하는 데 있어 최적의 방법이 될 수 없습니다.
