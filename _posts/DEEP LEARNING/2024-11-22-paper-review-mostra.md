...
1. **Counterfactual Filtering(ε)**
  * 곡의 점수가 $$ s_{\text{max}} - s_t \leq \epsilon $$를 넘지 못하면 필터링하여 제외합니다.
    * 예를 들어, 다음 곡을 선택할 때 SAT 점수가 일정 임계값(ε) 이하인 곡은 추천 후보에서 제외
      * 예시: $$s_{\text{max}}=0.95$$ & $$\varepsilon=0.05$$면 $$ s_t < 0.90 $$인 곡은 제외됨 <br>
...
1. **Multi-Objective 기반 순서 최적화**
  * Beam Search를 사용하여, 다음 단계에서 여러 후보 곡 조합을 평가한 후 가장 최적의 리스트를 생성합니다.
    * 현재 리스트에 새로운 곡을 추가할 때마다 전체 리스트의 점수를 재계산 <br>
...
2. **다양성 유지 및 Objectives 조화**
  * 곡 추천에서 특정 목표(Objective)에 편중되지 않도록, 리스트 내 곡들 간의 균형을 맞춥니다.
    * 즉, 이 과정에서 objective의 다양성을 고려하며, 특정 목표에 곡이 과도하게 몰리지 않도록 조정 <br>
...
이번 글에서는 기존 내용이 많다 보니 Set-Transformer와 Beam Search 알고리즘 같은 기본적인 설명은 건너 뛰었는데, 저도 이 부분을 더 공부하고 있어서 다음 포스팅에서 소개하는 것도 좋을 것 같네요. 여러분이 궁금한 점이나 아이디어가 있다면 꼭 공유해주세요! 🙌 
