...
음악 추천 시스템에서 가장 중요한 질문 중 하나는 **"사용자에게 어떤 곡을 왜 추천해야 할까?"**입니다. 이를 해결하기 위해 스포티파이는 사용자의 개별적인 취향과 스트리밍 행동 데이터를 기반으로 어떤 곡이 그들에게 적합할지 예측합니다. <br>
예를 들어, 사용자 A가 이전에 Track X와 Track Y를 스트리밍한 데이터가 있다면, 이는 사용자가 어떤 종류의 음악을 선호하는지에 대한 힌트를 제공합니다. 이러한 과거의 스트리밍 기록(historic interaction data)을 통해, 사용자 A가 Track Z와 같은 새로운 음악을 탐색할 가능성을 예측할 수 있습니다. 이를 Discovery objective로 정의하며, 이전에 들어본 적 없는 곡이나 아티스트를 추천해 사용자가 새로운 음악 세계를 경험하도록 돕습니다. <br>

또한 추천 시스템은 단순히 사용자 만족을 넘어 플랫폼의 전략적 목표도 고려해야 합니다. 특정 아티스트가 새로운 곡을 발표했거나, 주목받을 필요가 있는 신예 아티스트라면 어떻게 해야 할까요? 스포티파이는 이런 트랙에 Boost 태그를 부여합니다. <br>
예를 들어, 아티스트 B(Artist B)의 Track M이 플랫폼에서 중요한 곡으로 간주된다면, 이 곡은 Boost 태그가 붙고 추천 시스템에서 우선적으로 사용자에게 노출됩니다. 이 과정은 editorial annotations으로 이루어지며, 특정 곡이나 아티스트를 전략적으로 강조하기 위한 데이터를 제공합니다.
