...
1. **Counterfactual Filtering(ε)**
  * 곡의 점수가 $$ s_{\text{max}} - s_t \leq \epsilon $$를 넘지 못하면 필터링하여 제외합니다.
    ...
    * 예시: $$ s_{\text{max}}=0.95 & \varepsilon=0.05 $$면 $$ s_t < 0.90 $$인 곡 제외됨
...
2. **Submodular: MO(Multi-Objective) Beam Scoring**
  * 남은 후보 곡에 대해 곡의 **Relevance Score(rt)**와 **Objective 기반 점수(g(MO))**를 통합하여 최종 점수를 계산합니다.
    * s_{\text{final}} = s_t + g(\text{MO}) = s_t + \frac{1}{n} \sum_{j=1}^{J} \sqrt{\sum_{y \in [y \| t]} E_{y,j}}
      * $$s_t$$ : 기본 점수 (사용자 만족 기반 점수) & $$g(\text{MO})$$ : objective 기반 추가 점수 (서브모듈러 방식)
      * n : 현재 추천 리스트에서의 곡 순서 (예: 리스트의 세 번째 곡이면 n=3)
      * J : objective 총 개수
      * $$E_{y,j}$$ ​: objective j가 추천 리스트 내에서 얼마나 충족되었는지 누적값
...
# Conclusion
스포티파이는 단순히 사용자의 취향 데이터를 기반으로 곡을 추천하는 데 그치지 않고, 과거 스트리밍 기록과 같은 데이터(예: Discovery)를 활용하여 사용자가 새로운 음악을 발견할 수 있도록 돕습니다. 이와 동시에 Boost 태그와 같은 플랫폼 중심의 전략을 반영하여, 사용자, 아티스트, 플랫폼이라는 모든 이해관계자의 목표를 조화롭게 달성하고자 노력합니다. <br>
이번에 제시한 추천시스템은 **"좋아할 곡을 찾는 것"을 넘어**, 사용자가 새로운 음악을 탐색하고 아티스트의 가치를 극대화하며, 플랫폼의 지속 가능한 성장을 가능케 하는 매우 정교한 시스템이라 할 수 있습니다. <br>

#### 📍 Multi-Objective Recommendations
스포티파이의 접근 방식은 다중 이해관계자 플랫폼(multi-stakeholder digital platform)의 전형적인 특징을 보여줍니다. 음악 추천 분석은 다양한 objective 간의 복잡한 상호작용을 드러냈으며, 이러한 목표를 최적화하기 위한 다각적인 접근이 필요함을 시사합니다.

#### 📍 Just-in-Time Multi-Objective Optimization
이번 연구에서는 스포티파이의 Mostra 프레임워크가 다중 목표를 유연하게 최적화하는 데 중요한 역할을 수행함을 보여주었습니다. Mostra는 다음과 같은 특징을 갖습니다: <br>
* **End-to-End 방식**: 스케일 확장이 가능하고 유연한 Multi-Objective 추천시스템을 구현
* **빔 서치 알고리즘**: 다중 목표를 동적으로 최적화하는 과정에서 높은 유연성을 제공
* **State-of-the-Art(SOTA) 대비 성능 향상**: 아티스트 중심의 목표(creator-centric objectives)에서 SOTA를 초과하는 유의미한 성능 향상을 달성하며, 사용자 만족도 지표(user metric)에는 손실이 발생하지 않음
...
### 🔜 Think
이번 포스팅에서는 **Mostra 프레임워크**를 중심으로, 스포티파이가 다중 목표를 어떻게 조화롭게 달성하며 정교한 음악 경험을 제공하는지 살펴보았습니다. 🎧✨ <br>
특히 플랫폼의 전략적인 Boost 태그나, 사용자와 아티스트 간의 균형을 고려한 추천 방식을 보면서, **추천시스템이 단순히 사용자 만족을 넘어서 플랫폼의 지속 가능성까지 고민해야 하는 이유**를 다시 한번 느끼게 되었어요. 최근 **다양한 콘텐츠를 더 정교하게 추천하는 방법**에 대해 많은 고민을 하고 있었는데, 이런 접근 방식에서 많은 영감을 받았습니다. 🤔 <br>
이번 글에서는 기존 내용이 많다 보니 Set-Transformer와 Beam Search 알고리즘 같은 기본적인 설명은 건너 뛰었는데, 저도 이 부분을 더 공부하고 있어서 다음 포스팅에서 소개하는 것도 좋을 것 같네요. 여러분이 궁금한 점이나 아이디어가 있다면 꼭 공유해주세요! 🙌 <br>

또, 논문 저자가 해당 논문을 소개한 영상에서 앞으로의 추천시스템 확장 계획을 들을 수 있었는데, 이 부분도 정말 흥미로웠습니다!
1. **세션 간 objective(cross-session objectives) 최적화**: 단일 세션을 넘어 여러 세션에 걸친 장기적 목표 달성을 고려
2. **인과적 의사결정(causal decisioning)을 위한 학습 가능한 시스템 개발**: 데이터 기반으로 추천 결과와 사용자 행동 간의 인과 관계를 학습해, 더 정밀한 추천을 가능하게 하는 시스템

빠르게 바뀌는 세션의 특정을 잘 캐치하고 반영하는 1번 부분도 되지만, 개인적으로는 2번의 인과추론이 추천시스템에 어떻게 반영될지 정말 흥미로워 보입니다.👀📚 <br>

여러분은 Mostra 프레임워크의 접근 방식이 추천시스템이 있어 필수적이라고 생각하시나요? 다양한 의견을 함께 나눠보면 좋겠습니다!💫
