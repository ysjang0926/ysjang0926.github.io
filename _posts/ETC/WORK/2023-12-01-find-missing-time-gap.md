...
실제 분석을 진행할 때에는 실시간으로 데이터가 적재되는 DB에서 데이터를 추출하여 분석하면 안되겠죠? <br>
이러한 이유로 따로 분석용 DB를 따로 마련하여, 1초 단위로 쌓이는 데이터를 순시값으로 들고 와서 1분 데이터로 집계되도록 되어 있습니다.
...
1~2분 간격으로 간헐적으로 존재한다면 이를 무시하여 분석할 수도 있지만, 기기 이상 등 여러 요소들로 인해 **10분 이상의 데이터가 누락된 케이스도 존재**하게 됩니다.  <br>
이때 1분 집계 데이터에서 **이력이 끊기는 일자 및 시간대 집합**을 만들어서, **해당 범위의 1초 데이터를 들고 오려고 합니다.** (1초 데이터를 들고 온 뒤, 1분 데이터로 변환 예정)
![image](https://github.com/ysjang0926/ysjang0926.github.io/assets/54492747/1e3246f2-9f2c-46be-b89f-475844a4b7eb)
...
### 🔚 최종 SQL문
최종 SQL문은 다음과 같습니다. <br>
여기서 산출된 `MISSING_START_TIME` 변수와 `MISSING_END_TIME` 변수의 데이터 기간을 1초 Influx DB에서 Data I/F 작업을 진행하면 됩니다. <br>
**→ 누락된 기간만 1초 DB 재적재 진행**
```sql
...
### 🔜 1초 DB 재적재 진행
STEP1~STEP3까지가 아래의 그림에서 [**단절된 시간 범위 구하기**] 파트에 해당되는데요! <br>
이때 추출된 이력이 끊긴, 즉 누락된 시간대만큼 1초 Influx DB에서 데이터 재적재를 진행하게 됩니다. <br>
이 작업 또한 python으로 DB에 연결하여 데이터를 들고오는데, 해당 부분은 코드 내용이 다소 방대하여 이번 포스팅에서는 생략하도록 하겠습니다.
![image](https://github.com/ysjang0926/ysjang0926.github.io/assets/54492747/4d81c63a-567d-48f0-bc45-7fc271294093)
...
간단한 SQL문이지만 이런 케이스도 있는걸 소개하면 좋을 것 같아서 적어보았습니다. <br>
